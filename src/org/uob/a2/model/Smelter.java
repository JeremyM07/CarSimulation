...
public class Smelter extends Converter implements Tickable{
    
    private final ResourceType input2 = ResourceType.IRON_ORE;
    private final int input2Amount = 9;

    public Smelter(){
        super("Smelter", ResourceType.COAL_ORE, 4, ResourceType.STEEL, 15);
    ...
    public void convert(Context ctx){
        if (ctx.state().getResourceAmount(this.getInput()) >= this.getInputAmount() && ctx.state().getResourceAmount(this.getInput2()) >= this.getInput2Amount()){
