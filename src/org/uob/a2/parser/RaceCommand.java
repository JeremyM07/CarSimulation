...
public class RaceCommand extends Command {
    ...
    public String execute(Context ctx) {
            ...
            if (c instanceof RaceTrack) {
                this.level = ((RaceTrack) c).getLevel();
            } else {
                return "No Race Track found to race on!";
            }
            if (ctx.state().getInventory().get(ResourceType.RACE_CAR) < 1){
                return "No car to race!";
            }
